...
var logger = log.ChildLogger(log.RootLogger(), "activespaces-gateway-connector")
...
func (*ActiveSpacesFactory) Type() string { return "activespaces-gateway-connector" }
...
func (*ActiveSpacesFactory) NewManager(settings map[string]interface{}) (connection.Manager, error) {
	...
	connRef := s.Name
	if connRef == "" {
		connRef = fmt.Sprintf("activespaces-gateway-%s-%d", s.Host, s.Port)
	}
...
func (c *ActiveSpacesConnection) Type() string                       { return "activespaces-gateway-connector" }
