...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-count: invalid connection type, expected *AzureAISearchConnection")
	}
	ctx.Logger().Infof("CountDocuments initialised: connection=%s provider=%s", conn.GetName(), "azureaisearch")
