...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-hybrid: invalid connection type, expected *AzureAISearchConnection")
	}
	ctx.Logger().Infof("HybridSearch initialised: connection=%s provider=%s defaultTopK=%d",
		conn.GetName(), "azureaisearch", s.DefaultTopK)
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if alpha < 0 || alpha > 1 {
		return false, vectordb.NewError(vectordb.ErrCodeInvalidAlpha,
			fmt.Sprintf("alpha %.4f is out of range [0, 1]", alpha), nil)
	}
