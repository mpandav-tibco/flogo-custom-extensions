...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-get: invalid connection type, expected *LanceDBConnection")
	}
	ctx.Logger().Infof("GetDocument initialised: connection=%s provider=%s", conn.GetName(), "lancedb")
