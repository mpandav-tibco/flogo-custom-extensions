...
Insert or update documents with pre-computed embedding vectors in LanceDB.

Uses LanceDB `merge_insert` for atomic upsert semantics. Documents stored with fixed schema: `id`, `content`, `metadata` (JSON string), `embedding` (float32 array).
