...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-hybrid: invalid connection type, expected *LanceDBConnection")
	}
	ctx.Logger().Infof("HybridSearch initialised: connection=%s provider=%s defaultTopK=%d",
		conn.GetName(), "lancedb", s.DefaultTopK)
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	start := time.Now()
	// LanceDB handles hybrid search natively; no fallback needed.
	fallbackReason := ""
