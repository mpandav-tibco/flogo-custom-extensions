...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-drop-col: invalid connection type, expected *LanceDBConnection")
	}
	ctx.Logger().Infof("DeleteCollection initialised: connection=%s provider=%s", conn.GetName(), "lancedb")
