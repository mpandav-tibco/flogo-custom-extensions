...
// VectorDBClient is the unified interface for the LanceDB provider.
// All implementations must be safe for concurrent use by multiple goroutines.
type VectorDBClient interface {
	// --- Collection / Index Management ---

	// CreateCollection creates a new vector collection (LanceDB table).
	...
	// ListCollections returns the names of all collections in the LanceDB server.
	...
	// HybridSearch combines dense vector search with full-text search (RRF fusion).
	HybridSearch(ctx context.Context, req HybridSearchRequest) ([]SearchResult, error)

	// --- Lifecycle ---

	// HealthCheck verifies the LanceDB server is reachable and responsive.
	HealthCheck(ctx context.Context) error

	// DBType returns "lancedb".
	DBType() string
