    ...
    vectordbHandler = function (t) {
        function e(e, i) {
            ...
            n.action = function (actionName, connCtx) {
                // Native ActiveSpaces (tibdg) connections run in-process against the realm
                // and are not reachable from the designer host, so there is no design-time
                // connection test. This is a "Save" action: accept and persist the config
                // (the connection is validated at runtime inside the ActiveSpaces environment).
                if (actionName !== "Save") { return null; }
