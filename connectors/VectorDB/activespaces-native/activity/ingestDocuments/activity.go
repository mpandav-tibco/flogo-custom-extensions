...
type Activity struct {
	settings *Settings
	conn     *vectordbconnector.ActiveSpacesConnection
}
...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.ActiveSpacesConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-ingest: invalid connection type, expected *ActiveSpacesConnection")
	}
	...
	ctx.Logger().Infof("IngestDocuments initialised: connection=%s provider=%s embeddingProvider=%s model=%s chunking=%v strategy=%s",
		conn.GetName(), "activespaces", s.EmbeddingProvider, s.EmbeddingModel, s.EnableChunking, s.ChunkStrategy)
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if tc != nil {
		...
		tc.SetTag("db.vectordb.provider", "activespaces")
	...
	// Defensive: the embedding provider must return exactly one vector per text.
	// A mismatch (provider contract violation) would otherwise panic at allEmbeddings[i].
	if len(allEmbeddings) != len(rawDocs) {
		l.Errorf("IngestDocuments: embedding count mismatch: got %d vectors for %d documents", len(allEmbeddings), len(rawDocs))
		if err := ctx.SetOutputObject(&Output{
			Success:  false,
			Error:    fmt.Sprintf("embedding count mismatch: got %d vectors for %d documents", len(allEmbeddings), len(rawDocs)),
			Duration: time.Since(start).String(),
		}); err != nil {
			l.Errorf("SetOutputObject: %v", err)
		}
