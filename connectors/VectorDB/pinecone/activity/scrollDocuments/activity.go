...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-scroll: %w", err)
	}
	if s.Connection == nil {
		return nil, fmt.Errorf("vectordb-pinecone-scroll: connection is required")
	}
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PineconeConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-pinecone-scroll: invalid connection type, expected *PineconeConnection")
	}
	ctx.Logger().Infof("ScrollDocuments initialised: connection=%s provider=%s", conn.GetName(), "pinecone")
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-scroll: %w", err)
	}
	...
	if collectionName == "" {
		return false, fmt.Errorf("vectordb-pinecone-scroll: collectionName is required")
	}
