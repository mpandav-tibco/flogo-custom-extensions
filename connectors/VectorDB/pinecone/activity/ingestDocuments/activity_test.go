...
// newTestConn creates a PineconeConnection wired to the supplied mock client.
func newTestConn(client vectordb.VectorDBClient) *vectordbconnector.PineconeConnection {
	s := &vectordbconnector.Settings{
		PineconeCloud:  "aws",
...
// singleEmbedServer returns a test HTTP server that answers every embed request
// with exactly one embedding vector of `dims` dimensions.
...
// batchEmbedServer returns a test HTTP server that echoes back one embedding
// per input text, each of `dims` dimensions.
...
// getOutput extracts an Output struct from the raw map returned by SetOutputObject.
...
func TestIngestDocuments_EmbeddingFails(t *testing.T) {
	...
	assert.Contains(t, out.Error, "503")
}
...
func TestIngestDocuments_VectorDBFails(t *testing.T) {
	...
	out := getOutput(ctx.outputs)
	assert.False(t, out.Success)
	assert.Contains(t, out.Error, "upsert")
	mc.AssertExpectations(t)
}
