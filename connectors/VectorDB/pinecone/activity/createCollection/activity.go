...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-create-col: %w", err)
	}
	if s.Connection == nil {
		return nil, fmt.Errorf("vectordb-pinecone-create-col: connection is required")
	}
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PineconeConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-pinecone-create-col: invalid connection type, expected *PineconeConnection")
	}
	ctx.Logger().Infof("CreateCollection initialised: connection=%s provider=%s", conn.GetName(), "pinecone")
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-create-col: %w", err)
	}
	if input.CollectionName == "" {
		return false, fmt.Errorf("vectordb-pinecone-create-col: collectionName is required")
	}
	if input.Dimensions <= 0 {
		input.Dimensions = 1536 // default to OpenAI text-embedding-3-small
	}
	...
	l.Debugf("CreateCollection: name=%s dims=%d metric=%s",
		input.CollectionName, input.Dimensions, input.DistanceMetric)
	...
	if timeout <= 0 {
		timeout = 120 // collection creation + polling may take time for Pinecone
	}
