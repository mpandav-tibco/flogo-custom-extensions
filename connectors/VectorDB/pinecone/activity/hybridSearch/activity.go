...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-hybrid: %w", err)
	}
	if s.Connection == nil {
		return nil, fmt.Errorf("vectordb-pinecone-hybrid: connection is required")
	}
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PineconeConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-pinecone-hybrid: invalid connection type, expected *PineconeConnection")
	}
	ctx.Logger().Infof("HybridSearch initialised: connection=%s provider=%s defaultTopK=%d",
		conn.GetName(), "pinecone", s.DefaultTopK)
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-hybrid: %w", err)
	}
	...
	if collectionName == "" {
		return false, fmt.Errorf("vectordb-pinecone-hybrid: collectionName is required")
	}
	if input.QueryText == "" && len(input.QueryVector) == 0 {
		return false, fmt.Errorf("vectordb-pinecone-hybrid: at least one of queryText or queryVector must be provided")
	}
