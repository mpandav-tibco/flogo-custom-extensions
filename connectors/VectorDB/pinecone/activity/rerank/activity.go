...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-rerank: %w", err)
	}
	if s.RerankEndpoint == "" {
		return nil, fmt.Errorf("vectordb-pinecone-rerank: rerankEndpoint is required")
	}
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-rerank: %w", err)
	}
	if input.Query == "" {
		return false, fmt.Errorf("vectordb-pinecone-rerank: query is required")
	}
	if len(input.Documents) == 0 {
		return false, fmt.Errorf("vectordb-pinecone-rerank: documents must not be empty")
	}
