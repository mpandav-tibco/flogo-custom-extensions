...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-upsert: %w", err)
	}
	if s.Connection == nil {
		return nil, fmt.Errorf("vectordb-pinecone-upsert: connection is required")
	}
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PineconeConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-pinecone-upsert: invalid connection type, expected *PineconeConnection")
	}
	ctx.Logger().Infof("UpsertDocuments initialised: connection=%s provider=%s", conn.GetName(), "pinecone")
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-upsert: %w", err)
	}
	...
	if collectionName == "" {
		return false, fmt.Errorf("vectordb-pinecone-upsert: collectionName is required")
	}
