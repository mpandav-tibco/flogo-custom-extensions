...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-rag: %w", err)
	}
	if s.Connection == nil {
		return nil, fmt.Errorf("vectordb-pinecone-rag: connection is required")
	}
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PineconeConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-pinecone-rag: invalid connection type, expected *PineconeConnection")
	}
	...
	if s.EmbeddingModel == "" {
		return nil, fmt.Errorf("vectordb-pinecone-rag: embeddingModel is required")
	}
	if s.DefaultTopK <= 0 {
		s.DefaultTopK = 5
	}
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-rag: %w", err)
	}
	if input.QueryText == "" {
		return false, fmt.Errorf("vectordb-pinecone-rag: queryText is required")
	}
	...
	if collectionName == "" {
		return false, fmt.Errorf("vectordb-pinecone-rag: collectionName is required")
	}
	...
	if a.settings.UseHybridSearch {
		...
		if alpha < 0 || alpha > 1 {
			return false, fmt.Errorf("vectordb-pinecone-rag: hybridAlpha must be between 0.0 and 1.0, got %.4f", alpha)
		}
