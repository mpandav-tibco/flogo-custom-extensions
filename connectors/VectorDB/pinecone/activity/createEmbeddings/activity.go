...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-embed: %w", err)
	}
	...
	if s.UseConnectorEmbedding && s.Connection != nil {
		...
		if !ok {
			return nil, fmt.Errorf("vectordb-pinecone-embed: invalid connection type, expected *PineconeConnection")
		}
	...
	if s.Model == "" {
		return nil, fmt.Errorf("vectordb-pinecone-embed: model is required")
	}
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-embed: %w", err)
	}
	...
	if len(texts) == 0 {
		return false, fmt.Errorf("vectordb-pinecone-embed: inputText or inputTexts is required")
	}
