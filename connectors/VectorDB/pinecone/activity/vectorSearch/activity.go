...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if err := metadata.MapToStruct(ctx.Settings(), s, true); err != nil {
		return nil, fmt.Errorf("vectordb-pinecone-search: %w", err)
	}
	if s.Connection == nil {
		return nil, fmt.Errorf("vectordb-pinecone-search: connection is required")
	}
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PineconeConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-pinecone-search: invalid connection type, expected *PineconeConnection")
	}
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if err := ctx.GetInputObject(input); err != nil {
		return false, fmt.Errorf("vectordb-pinecone-search: %w", err)
	}
	...
	if collectionName == "" {
		return false, fmt.Errorf("vectordb-pinecone-search: collectionName is required")
	}
	if len(input.QueryVector) == 0 {
		return false, fmt.Errorf("vectordb-pinecone-search: queryVector must not be empty")
	}
