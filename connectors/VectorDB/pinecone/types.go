...
type Document struct {
	// ID is the unique identifier. Use UUIDs for best compatibility.
	ID string `json:"id"`
...
type CollectionConfig struct {
	...
	// OnDisk controls on-disk vector storage (reduces RAM usage; not used by Pinecone serverless).
	OnDisk bool

	// ReplicationFactor sets the replica count. Not used by Pinecone serverless.
	ReplicationFactor int
}

// SearchRequest encapsulates a vector similarity search.
type SearchRequest struct {
	// CollectionName is the target collection (Pinecone index name).
...
// ConnectionConfig holds all settings needed to connect to Pinecone.
type ConnectionConfig struct {
	// DBType identifies this as a Pinecone connection. Always "pinecone".
	DBType string

	// APIKey for Pinecone authentication (required).
	APIKey string

	// Host is the Pinecone control-plane hostname.
	// Default: "api.pinecone.io"
	Host string

	// PineconeCloud is the cloud provider for serverless indexes: "aws", "gcp", "azure".
	// Default: "aws"
	PineconeCloud string

	// PineconeRegion is the cloud region for serverless indexes.
	// Default: "us-east-1"
	...
	// Scheme is the URL scheme for the control-plane API: "https" (default) or "http".
	// Set to "http" when using Pinecone Local (ghcr.io/pinecone-io/pinecone-local).
	Scheme string
}
...
func (c ConnectionConfig) String() string {
	...
	return fmt.Sprintf(
		"ConnectionConfig{DBType:%q Host:%q Cloud:%q Region:%q APIKey:%s Timeout:%d}",
		c.DBType, c.Host, c.PineconeCloud, c.PineconeRegion, apiKey, c.TimeoutSeconds,
	)
}
...
type VectorDBClient interface {
	...
	// report a count. Callers must not treat -1 as an error; use CountDocuments
	// before/after if an exact count is required.
	...
	// DBType returns the provider identifier: "pinecone".
	DBType() string
