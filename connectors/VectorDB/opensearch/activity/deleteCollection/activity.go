...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-drop-col: invalid connection type, expected *OpenSearchConnection")
	}
	ctx.Logger().Infof("DeleteCollection initialised: connection=%s provider=%s", conn.GetName(), "opensearch")
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	l.Warnf("DeleteCollection: PERMANENTLY deleting collection=%s from provider=%s",
		input.CollectionName, "opensearch")
