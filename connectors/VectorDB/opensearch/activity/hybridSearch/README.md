...
Combined dense vector + keyword search in OpenSearch 2.x with configurable alpha blend. Returns results fused from both retrieval modes.

**Implementation**: Native: k-NN `knn_vector` + BM25 `multi_match` via `bool/should`, blended with `alpha`. Filters applied as `bool/filter` (no scoring impact).
...
Native: k-NN `knn_vector` + BM25 `multi_match` via `bool/should`, blended with `alpha`. Filters applied as `bool/filter` (no scoring impact).
