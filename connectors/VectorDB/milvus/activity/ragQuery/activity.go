...
import (
	...
	vdbembed "github.com/mpandav-tibco/flogo-extensions/connectors/VectorDB/milvus/embeddings"
...
type Activity struct {
	settings *Settings
	conn     *vectordbconnector.MilvusConnection
}
...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.MilvusConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-rag: invalid connection type, expected *MilvusConnection")
	}
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if tc != nil {
		...
		tc.SetTag("db.vectordb.provider", "milvus")
