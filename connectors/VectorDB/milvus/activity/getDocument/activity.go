...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-get: invalid connection type, expected *MilvusConnection")
	}
	ctx.Logger().Infof("GetDocument initialised: connection=%s provider=%s", conn.GetName(), "milvus")
