...
// NewClient creates the Redis VectorDBClient for the given ConnectionConfig.
...
func validateConnectionConfig(cfg *ConnectionConfig) error {
	...
	if cfg.Port > 65535 {
		return newError(ErrCodeInvalidPort, fmt.Sprintf("port %d out of range 1-65535", cfg.Port), nil)
	}
	if cfg.RedisDB < 0 || cfg.RedisDB > 15 {
		return newError(ErrCodeInvalidPort, fmt.Sprintf("redisDB %d out of range 0-15", cfg.RedisDB), nil)
	}
	if cfg.MaxRetries == 0 {
		cfg.MaxRetries = 3
	}
