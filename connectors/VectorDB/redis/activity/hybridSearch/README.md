...
Combined dense vector + keyword search in Redis Stack (RediSearch) with configurable alpha blend. Returns results fused from both retrieval modes.

**Implementation**: Native: HNSW KNN + BM25 full-text in a single `FT.SEARCH` query, blended with `alpha`
...
Native: HNSW KNN + BM25 full-text in a single `FT.SEARCH` query, blended with `alpha`
