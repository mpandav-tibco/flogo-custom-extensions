...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-scroll: invalid connection type, expected *RedisConnection")
	}
	ctx.Logger().Infof("ScrollDocuments initialised: connection=%s provider=%s", conn.GetName(), "redis")
