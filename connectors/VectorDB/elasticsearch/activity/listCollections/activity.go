...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-list-col: invalid connection type, expected *ElasticsearchConnection")
	}
	ctx.Logger().Infof("ListCollections initialised: connection=%s provider=%s", conn.GetName(), "elasticsearch")
