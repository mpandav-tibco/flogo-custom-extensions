...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-delete: invalid connection type, expected *ElasticsearchConnection")
	}
	ctx.Logger().Infof("DeleteDocuments initialised: connection=%s provider=%s", conn.GetName(), "elasticsearch")
