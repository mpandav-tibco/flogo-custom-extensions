...
Count the number of documents in a Elasticsearch 8.x collection, with optional metadata filtering.
...
Elasticsearch `bool/must` queries mapped from MongoDB-style operators; supports `$in`/`$nin`
...
- Filter semantics: Elasticsearch `bool/must` queries mapped from MongoDB-style operators; supports `$in`/`$nin`.
