...
Combined dense vector + keyword search in Elasticsearch 8.x with configurable alpha blend. Returns results fused from both retrieval modes.

**Implementation**: Native: k-NN `dense_vector` + BM25 `multi_match` in a single query, blended with `alpha`. Filters applied to both legs.
...
Native: k-NN `dense_vector` + BM25 `multi_match` in a single query, blended with `alpha`. Filters applied to both legs.
