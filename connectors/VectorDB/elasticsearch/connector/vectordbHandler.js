...
var core_1 = require("@angular/core"),
    ...
    TLS_FIELDS = ["tlsInsecureSkipVerify"],
        ...
        function e(e, i) {
            ...
            n.action = function (actionName, connCtx) {
                ...
                return n.toSettingsObj(connCtx.settings).switchMap(function (s) {
                    ...
                    var port = parseInt(s.port, 10) || 9200;
                    ...
                    return new rxjs_extensions_1.Observable(function (observer) {
                        window.fetch(url, { method: "GET", mode: "no-cors", cache: "no-cache" })
                            .then(function (_resp) {
                                observer.next(contrib_1.ActionResult.newActionResult().setResult({ context: connCtx, authType: wi_contrib_1.AUTHENTICATION_TYPE.BASIC, authData: {} }));
                                observer.complete();
                            })
                            .catch(function (err) {
                                var msg = (err && err.message) ? err.message : ("unreachable: " + url);
                                observer.next(contrib_1.ActionResult.newActionResult().setSuccess(false).setResult(new validation_1.ValidationError("VDB-1001", "Connection failed: " + msg)));
                                observer.complete();
                            });
                    });
                });
