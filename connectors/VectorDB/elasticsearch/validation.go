...
// Batch size limit for elasticsearch.
const (
	maxUpsertBatchElasticsearch = 1_000
)
...
// ValidateCollectionConfig is the exported wrapper for use in tests and external packages.
func ValidateCollectionConfig(cfg CollectionConfig) error {
	return validateCollectionConfig(cfg)
}

// ValidateSearchRequest is the exported wrapper for use in tests and external packages.
func ValidateSearchRequest(req SearchRequest) error {
	return validateSearchRequest(req.CollectionName, req.QueryVector, req.TopK)
}
