...
- **Use `pureGo`** for portability, simplest app builds, and when you can run one small gateway
  container next to the grid (or share one across many apps). This is the recommended default.
- **Use `nativeAS`** when you want to eliminate the gateway hop, are already deploying on
