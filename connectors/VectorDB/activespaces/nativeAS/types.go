...
type ConnectionConfig struct {
	...
	// Host is the ActiveSpaces realm (FTL) server hostname or IP.
	Host string

	// Port is the ActiveSpaces realm server port. Default: 8080.
	Port int

	// APIKey is an optional credential (reserved; the native tibdg client
	// authenticates via the realm/FTL secure connection, not a bearer token).
	APIKey string
