...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-hybrid: invalid connection type, expected *ActiveSpacesConnection")
	}
	ctx.Logger().Infof("HybridSearch initialised: connection=%s provider=%s defaultTopK=%d",
		conn.GetName(), "activespaces", s.DefaultTopK)
