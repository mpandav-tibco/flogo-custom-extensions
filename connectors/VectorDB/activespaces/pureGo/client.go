...
//
// Supported providers: activespaces, weaviate, chroma, milvus
type VectorDBClient interface {
	...
	// report a count (ActiveSpaces, Weaviate, Chroma, Milvus). Callers must not treat -1 as an
	...
	// DBType returns the provider identifier: "activespaces", "weaviate", "chroma", or "milvus".
	DBType() string
