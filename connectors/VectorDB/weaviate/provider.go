...
func (c *weaviateClient) CreateCollection(ctx context.Context, cfg CollectionConfig) error {
	...
	classObj := &models.Class{
		...
		Properties: []*models.Property{
			...
			// _docId must use "field" tokenization so that filters.Equal performs
			// exact string matching (not word-tokenized matching).  With default
			// "word" tokenization, Equal("_docId", "user-123") would tokenise the
			// query to ["user","123"] and match any object whose _docId contains
			// BOTH tokens — potentially deleting or returning wrong documents.
			{Name: "_docId", DataType: []string{"text"}, Tokenization: models.PropertyTokenizationField},
