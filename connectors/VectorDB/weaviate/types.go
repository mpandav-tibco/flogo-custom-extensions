...
type ConnectionConfig struct {
	// DBType identifies this as a Weaviate connection. Always "weaviate".
	DBType string

	// Host is the Weaviate server hostname or IP.
	Host string

	// Port is the Weaviate HTTP port. Default: 8080.
	Port int

	// APIKey for Weaviate Cloud authentication.
	APIKey string
...
func (c ConnectionConfig) String() string {
	...
	return fmt.Sprintf(
		"ConnectionConfig{DBType:%q Host:%q Port:%d Scheme:%q APIKey:%s UseTLS:%v ClientKey:%s}",
		c.DBType, c.Host, c.Port, c.Scheme, apiKey, c.UseTLS, clientKey,
	)
}
