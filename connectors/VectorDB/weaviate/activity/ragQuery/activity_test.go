...
import (
	...
	mockclient "github.com/mpandav-tibco/flogo-extensions/connectors/VectorDB/weaviate/testutil/mock"
...
func newTestConn(client vectordb.VectorDBClient) *vectordbconnector.WeaviateConnection {
	s := &vectordbconnector.Settings{Host: "localhost", TimeoutSeconds: 5}
...
func TestRAGQuery_EmbeddingFails(t *testing.T) {
	...
	errStr, _ := ctx.outputs["error"].(string)
	assert.Contains(t, errStr, "embedding")
}
...
func TestRAGQuery_VectorSearchFails(t *testing.T) {
	...
	assert.Equal(t, false, ctx.outputs["success"])
	errStr, _ := ctx.outputs["error"].(string)
	assert.Contains(t, errStr, "vector search failed")
	mc.AssertExpectations(t)
}

func TestRAGQuery_HybridSearchFails(t *testing.T) {
	vec := []float64{0.1, 0.2}
	embedServer := makeEmbedServer(vec)
	defer embedServer.Close()

	mc := &mockclient.VectorDBClient{}
	mc.On("HybridSearch", mock.Anything, mock.Anything).Return(
		[]vectordb.SearchResult(nil), fmt.Errorf("hybrid index unavailable"))
	...
	a := &Activity{
		...
		settings: &Settings{
			EmbeddingProvider: "OpenAI", EmbeddingBaseURL: embedServer.URL,
			EmbeddingModel: "text-embedding-3-small", DefaultCollection: "col",
			DefaultTopK: 5, ContentField: "text", ContextFormat: "numbered",
			TimeoutSeconds: 5, UseHybridSearch: true, HybridAlpha: 0.5,
		},
	}
	ctx := &fakeActivityContext{inputs: map[string]interface{}{"queryText": "query"}}
	...
	assert.NoError(t, err)
	assert.Equal(t, false, ctx.outputs["success"])
	errStr2, _ := ctx.outputs["error"].(string)
	assert.Contains(t, errStr2, "hybrid search failed")
	mc.AssertExpectations(t)
}

func TestRAGQuery_HybridAlphaInvalid(t *testing.T) {
	vec := []float64{0.1, 0.2}
	embedServer := makeEmbedServer(vec)
	defer embedServer.Close()
	...
	a := &Activity{
		...
		settings: &Settings{
			...
			EmbeddingModel: "text-embedding-3-small", DefaultCollection: "col",
			DefaultTopK: 5, ContentField: "text", ContextFormat: "numbered",
			TimeoutSeconds: 5, UseHybridSearch: true, HybridAlpha: 1.5,
		},
	}
	ctx := &fakeActivityContext{inputs: map[string]interface{}{"queryText": "query"}}
	...
	assert.False(t, ok)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "hybridAlpha")
}
