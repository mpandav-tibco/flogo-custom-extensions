...
type ConnectionConfig struct {
	// DBType identifies this as an Qdrant connection. Always "qdrant".
	DBType string

	// Host is the Qdrant server hostname or IP.
	Host string

	// Port is the Qdrant REST API port. Default: 6333.
	Port int

	// APIKey for Qdrant Cloud authentication.
	APIKey string
...
func (c ConnectionConfig) String() string {
	...
	return fmt.Sprintf(
		"ConnectionConfig{DBType:%q Host:%q Port:%d GRPCPort:%d APIKey:%s UseTLS:%v ClientKey:%s}",
		c.DBType, c.Host, c.Port, c.GRPCPort, apiKey, c.UseTLS, clientKey,
	)
}
