...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-upsert: invalid connection type, expected *QdrantConnection")
	}
	ctx.Logger().Infof("UpsertDocuments initialised: connection=%s provider=%s", conn.GetName(), "qdrant")
