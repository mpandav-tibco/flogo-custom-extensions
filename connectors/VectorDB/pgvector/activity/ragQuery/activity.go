...
import (
	...
	"github.com/mpandav-tibco/flogo-extensions/connectors/VectorDB/pgvector"
	vectordbconnector "github.com/mpandav-tibco/flogo-extensions/connectors/VectorDB/pgvector/connector"
	vdbembed "github.com/mpandav-tibco/flogo-extensions/connectors/VectorDB/pgvector/embeddings"
	"github.com/project-flogo/core/activity"
	"github.com/project-flogo/core/data/metadata"
)

// ragLLMHTTPClient is a package-level HTTP client with explicit transport
// timeouts for LLM generation calls. http.DefaultClient has no dial or
// TLS-handshake timeout, which risks goroutine leaks on slow LLM endpoints.
var ragLLMHTTPClient = &http.Client{
	Transport: &http.Transport{
		...
		ResponseHeaderTimeout: 120 * time.Second, // LLM inference can be slow
		MaxIdleConns:          5,
...
type Activity struct {
	settings *Settings
	conn     *vectordbconnector.PgvectorConnection
}
...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	conn, ok := s.Connection.GetConnection().(*vectordbconnector.PgvectorConnection)
	if !ok {
		return nil, fmt.Errorf("vectordb-rag: invalid connection type, expected *PgvectorConnection")
	}
...
func (a *Activity) Eval(ctx activity.Context) (bool, error) {
	...
	if tc != nil {
		...
		tc.SetTag("db.vectordb.provider", "pgvector")
...
func (a *Activity) generateOllama(ctx context.Context, prompt string) (string, error) {
	...
	resp, err := ragLLMHTTPClient.Do(httpReq)
	if err != nil {
		return "", fmt.Errorf("ollama: http: %w", err)
	}
	defer resp.Body.Close()

	// Limit to 10 MB to prevent unbounded memory allocation from a large response.
