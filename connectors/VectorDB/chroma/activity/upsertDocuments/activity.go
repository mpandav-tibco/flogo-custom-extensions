...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if !ok {
		return nil, fmt.Errorf("vectordb-upsert: invalid connection type, expected *ChromaConnection")
	}
	ctx.Logger().Infof("UpsertDocuments initialised: connection=%s provider=%s", conn.GetName(), "chroma")
