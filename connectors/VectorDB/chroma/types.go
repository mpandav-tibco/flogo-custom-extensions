...
type ConnectionConfig struct {
	// DBType identifies this as a Chroma connection. Always "chroma".
	DBType string

	// Host is the Chroma server hostname or IP.
	Host string

	// Port is the Chroma HTTP port. Default: 8000.
	Port int

	// APIKey for Chroma authentication (optional).
	APIKey string
	...
	// Scheme is the HTTP scheme: "http" or "https". Default: "http" (or "https" when UseTLS=true).
	Scheme string
...
func (c ConnectionConfig) String() string {
	...
	return fmt.Sprintf(
		"ConnectionConfig{DBType:%q Host:%q Port:%d APIKey:%s UseTLS:%v ClientKey:%s}",
		c.DBType, c.Host, c.Port, apiKey, c.UseTLS, clientKey,
	)
}
